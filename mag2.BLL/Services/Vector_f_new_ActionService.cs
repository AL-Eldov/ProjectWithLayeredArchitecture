...
public class Vector_f_new_ActionService : IActionService<Vector_f_new_DTO>
{
    ...
    public async Task<Vector_f_new_DTO> Get(int id)
    {
        var vector_f_new = await db.Vector_f_new_Values.Get(id);
        return new Vector_f_new_DTO { Id = vector_f_new.Id, RealPart = vector_f_new.RealPart, ImaginaryPart = vector_f_new.ImaginaryPart };
    }
    public async Task Create(Vector_f_new_DTO item)
    {
        await db.Vector_f_new_Values.Create(new Vector_f_new { RealPart = item.RealPart, ImaginaryPart = item.ImaginaryPart });
        await db.Save();
    }
    public async Task CreateWithoutSave(Vector_f_new_DTO item)
    {
        await db.Vector_f_new_Values.Create(new Vector_f_new { RealPart = item.RealPart, ImaginaryPart = item.ImaginaryPart });
    }
    public async Task Update(Vector_f_new_DTO item)
    {
